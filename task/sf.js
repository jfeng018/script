...
function loginapp() {
  ...
  // 【核心修复】清洗请求头，剔除会导致 Surge 冲突的字段并统一化
  const cleanHeaders = {}
  for (let key in loginOpts.headers) {
    const lowerKey = key.toLowerCase()
    // 排除所有可能引起冲突的原始大小写混乱字段
    if (!['cookie', 'content-length', 'content-type', 'accept-encoding'].includes(lowerKey)) {
      cleanHeaders[key] = loginOpts.headers[key]
    }
  }
  // 强制补全标准 Header，防止 Env 类自动添加导致双重 Header
  cleanHeaders['Content-Type'] = 'application/json'
  loginOpts.headers = cleanHeaders
...
function getPoint(task) {
  return $.http
    .post({
      ...
      body: JSON.stringify({"strategyId": task.strategyId, "taskId": task.taskId, "taskCode": task.taskCode, "channelType":"1"}),
      headers: { 'Content-Type': 'application/json' }
    })
    .then((resp) => {
...
function showmsg() {
  ...
  $.desc = []
  if (success) {
    $.subt += $.sign.obj.hasFinishSign ? `重复` : `成功`
    $.desc.push(`说明: 连续签到${$.sign.obj.countDay}天`)
  } else {
    $.subt += `失败`
    $.desc.push(`说明: ${$.sign ? $.sign.errorMessage : '未知错误'}`)
  }
  $.desc.push('', `每日任务: `)
  for (let i = 0; i < $.tasks.length; i++) {
    $.desc.push(`${$.tasks[i].title}: ${$.tasks[i].result}`)
  }
